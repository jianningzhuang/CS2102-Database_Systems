...
--psql -U postgres -d project_db -f schema_W12_Monday.sql
--psql -U postgres -d project_db -f proc_W12_Monday.sql
--psql -U postgres -d project_db -f data_W12_Monday.sql
...
DECLARE
  fever BOOLEAN;
BEGIN
  IF employee_id in (SELECT eid FROM HealthDeclarations WHERE decl_date = declare_date) THEN
    RAISE EXCEPTION 'Temperature for employee % already declared on %', employee_id, decl_date;
  END IF;

  IF temperature < 34 OR temperature > 43 THEN
    RAISE EXCEPTION 'Temperature must be in the range 34-43';
  END IF;
  ...
  ELSE
    RAISE EXCEPTION 'Declaration cannot be in the future';
